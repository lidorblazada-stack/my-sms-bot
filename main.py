...
# הגדרות IDs - מבוסס על מה ששלחת לי
...
class GuardBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print("🛡️ שומר השרת מאובטח ומוכן לפעולה!")
...
async def clear_warns(interaction: discord.Interaction, member: discord.Member):
    await fb_request("DELETE", f"warnings/{member.id}")
    await interaction.response.send_message(f"✅ האזהרות של {member.name} אופסו בהצלחה.")

@bot.tree.command(name="block_user", description="חסימת משתמש ממערכת הדיווחים או ההמלצות")
...
async def block_user(interaction: discord.Interaction, member: discord.Member, system: str):
    ...
    await interaction.response.send_message(f"🚫 {member.name} נחסם משימוש במערכת ה-{system}.", ephemeral=True)
...
async def report(interaction: discord.Interaction, member: discord.Member, reason: str):
    ...
    if is_blocked:
        return await interaction.response.send_message("❌ אתה חסום משימוש במערכת הדיווחים.", ephemeral=True)
    
    channel = bot.get_channel(REPORT_CHANNEL_ID)
    if channel:
        embed = discord.Embed(title="🚨 דיווח חדש הגיע", color=0xff0000, timestamp=datetime.datetime.now())
        # סדר נכון לעברית: קודם מאת ואז על מי
...
async def suggest(interaction: discord.Interaction, suggestion: str):
    ...
    if is_blocked:
        return await interaction.response.send_message("❌ אתה חסום משימוש במערכת ההמלצות.", ephemeral=True)
...
async def on_member_join(member):
    ...
    if channel:
        embed = discord.Embed(
            title="🎊 ברוך הבא לשרת! 🎊",
            description=f"שלום {member.mention}, הגעת לשרת המוגן שלנו! תהנה מהשהות 🛡️",
            color=0x3498db,
...
async def on_member_remove(member):
    ...
    if channel:
        embed = discord.Embed(
            title="👋 להתראות",
            description=f"המשתמש **{member.name}** עזב את השרת. נתגעגע! 😢",
            color=0xff0000,
            timestamp=datetime.datetime.now()
        )
        embed.set_thumbnail(url=member.avatar.url if member.avatar else member.default_avatar.url)
