...
def is_owner_or_jail_staff(i: discord.Interaction):
    ...
    if role in i.user.roles: return True
    return False
...
class ShopView(ui.View):
    ...
    async def check_bal(self, i, b): await i.response.send_message(f"💰 היתרה הנוכחית שלך עומדת על: **₪{user_balances.get(i.user.id, 0):,}**", ephemeral=True)

    @ui.button(label="🛠️ צא לעבודה (Work)", style=discord.ButtonStyle.primary, custom_id="sh_work", row=0)
    async def do_work(self, i, b):
        now = datetime.now(); last = work_cooldown.get(i.user.id)
        if last and now < last + timedelta(minutes=10):
            diff = (last + timedelta(minutes=10)) - now
            return await i.response.send_message(f"❌ המשמרת שלך נגמרה! חזור בעוד {diff.seconds // 60} דקות.", ephemeral=True)
        amt = random.randint(200, 600); user_balances[i.user.id] = user_balances.get(i.user.id, 0) + amt
    ...
    async def claim_daily(self, i, b):
        now = datetime.now(); last = daily_cooldown.get(i.user.id)
        if last and now < last + timedelta(days=1):
            diff = (last + timedelta(days=1)) - now; hours, remainder = divmod(diff.seconds, 3600); minutes = remainder // 60
            return await i.response.send_message(f"❌ כבר אספת את הפרס יומי! חזור בעוד {hours} שעות ו-{minutes} דקות.", ephemeral=True)
        amt = random.randint(500, 1500); user_balances[i.user.id] = user_balances.get(i.user.id, 0) + amt
    ...
    async def process_purchase(self, i, price, role_id, role_name):
        ...
        user_balances[i.user.id] -= price; await i.user.add_roles(role)
...
class PoliceView(ui.View):
    def __init__(self, robber):
        super().__init__(timeout=10); self.robber = robber; self.called = False
...
class HeistView(ui.View):
    ...
    async def rob_user(self, i, b):
        ...
        async def callback(inter):
            ...
            if not p_view.called:
                ...
                if target_bal <= 0: return await inter.followup.send(f"❌ השוד נכשל! הקורבן {target.name} תפרן.", ephemeral=True)
    ...
    async def rob_bank(self, i, b):
        if i.user.id in jail_list: return await i.response.send_message("❌ אתה בכלא, תשתחרר קודם!", ephemeral=True)
    ...
    async def bail_friend(self, i, b):
        ...
        async def callback(select_inter):
            select.disabled = True; await select_inter.message.edit(view=view)
...
class CyberMasterBot(commands.Bot):
    ...
    async def jail_loop(self):
        now = datetime.now(); to_release = [uid for uid, release_time in jail_list.items() if now >= release_time]
...
async def s_shop(i):
    ...
    await i.channel.send(embed=emb, view=ShopView()); await i.response.send_message("✅ פאנל חנות משודרג הוקם בהצלחה!", ephemeral=True)
...
async def s_leaderboard(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "setup_leaderboard")]; return await i.response.send_message("❌ פקודה זו חסומה עבורך.", ephemeral=True)
    ...
    msg = await i.channel.send(embed=emb); leaderboard_config["channel_id"] = i.channel.id; leaderboard_config["message_id"] = msg.id
...
async def s_heist(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "setup_heist")]; return await i.response.send_message("❌ פקודה זו חסומה עבורך.", ephemeral=True)
    ...
    await i.channel.send(embed=emb, view=HeistView()); await i.response.send_message("✅ פאנל שודים הוקם בהצלחה!", ephemeral=True)
...
async def s_tickets(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "setup_tickets")]; return await i.response.send_message("❌ פקודה זו חסומה עבורך.", ephemeral=True)
    v = ui.View(timeout=None); b_rep = ui.Button(label="🚨 דווח על שחקן", style=discord.ButtonStyle.danger, custom_id="tk_rep"); b_sug = ui.Button(label="💡 שלח הצעה לשיפור", style=discord.ButtonStyle.secondary, custom_id="tk_sug")
    b_rep.callback = lambda inter: inter.response.send_modal(ReportModal()); b_sug.callback = lambda inter: inter.response.send_modal(SuggestionModal()); v.add_item(b_rep); v.add_item(b_sug)
...
async def s_feedback(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "setup_feedback")]; return await i.response.send_message("❌ פקודה זו חסומה עבורך.", ephemeral=True)
...
async def s_verify(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "setup_verify")]; return await i.response.send_message("❌ פקודה זו חסומה עבורך.", ephemeral=True)
...
async def jail_remove(i, member: discord.Member):
    if not is_owner_or_jail_staff(i): [await send_unauthorized_alert(i, "jail_remove", "צוות כלא / מנהל / אונר")]; return await i.response.send_message("❌ שגיאה: פקודה זו חסומה עבורך.", ephemeral=True)
    if member.id in jail_list: del jail_list[member.id]; await i.response.send_message(f"✅ הפקודה בוצעה בהצלחה! 🔓 המשתמש {member.mention} שוחרר מהכלא על ידי צוות המערכת.")
...
async def warn(i, member: discord.Member, reason: str):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "warn")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    user_warns[member.id] = user_warns.get(member.id, 0) + 1; count = user_warns[member.id]; log = i.guild.get_channel(CHANNELS["WARNS_LOG"])
    await log.send(f"⚠️ **אזהרה רשמית** | {member.mention} הוזהר על ידי {i.user.mention}.\nסיבה: `{reason}`\nמצב אזהרות: `{count}/3`")
    if count >= 3: [await member.add_roles(i.guild.get_role(ROLES["MUTE"])), await log.send(f"🚫 המשתמש {member.mention} הגיע ל-3 אזהרות והושתק (Mute) אוטומטית.")]
...
async def unwarn(i, member: discord.Member):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "unwarn")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    ...
    user_warns[member.id] -= 1; await i.response.send_message(f"✅ הורדה אזהרה בהצלחה. מצבו הנוכחי: `{user_warns[member.id]}/3` אזהרות.", ephemeral=True)
...
async def mute(i, member: discord.Member, reason: str):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "mute")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    await member.add_roles(i.guild.get_role(ROLES["MUTE"])); await i.response.send_message(f"🚫 המשתמש {member.mention} הושתק בהצלחה מהצ'אטים. סיבה: `{reason}`")
...
async def unmute(i, member: discord.Member):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "unmute")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    await member.remove_roles(i.guild.get_role(ROLES["MUTE"])); await i.response.send_message(f"🔊 המשתמש {member.mention} שוחרר מההשתקה ויכול לדבר.")
...
async def kick(i, member: discord.Member, reason: str):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "kick")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    await member.kick(reason=reason); await i.response.send_message(f"👞 המשתמש `{member.name}` הועף מהשרת בהצלחה. סיבה: `{reason}`")
...
async def ban(i, member: discord.Member, reason: str):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "ban")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    await member.ban(reason=reason); await i.response.send_message(f"🚫 המשתמש `{member.name}` נחסם מהשרת בהצלחה ובאופן מוחלט. סיבה: `{reason}`")
...
async def clear(i, amount: int):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "clear")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    if amount <= 0: return await i.response.send_message("❌ נא להזין מספר הודעות הגבוה מ-0.", ephemeral=True)
    await i.channel.purge(limit=amount); await i.response.send_message(f"🗑️ הערוץ נוקה בהצלחה! נמחקו `{amount}` הודעות אחרונות.", ephemeral=True)
...
async def slowmode(i, seconds: int):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "slowmode")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    await i.channel.edit(slowmode_delay=seconds); await i.response.send_message(f"⏱️ הוגגר מצב איטי לערוץ זה בהצלחה. דיליי: `{seconds}` שניות.")
...
async def add_money(i, member: discord.Member, amount: int):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "add_money")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    user_balances[member.id] = user_balances.get(member.id, 0) + amount; await i.response.send_message(f"💰 הדפסת בהצלחה **₪{amount:,}** והפקדת אותם לחשבון של {member.mention}!", ephemeral=True)
...
async def user_info(i, member: discord.Member):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "user_info")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
    emb = discord.Embed(title=f"👤 מידע על: {member.name}", color=0x5865f2); emb.add_field(name="מזהה (ID):", value=member.id, inline=True); emb.add_field(name="יתרת בנק:", value=f"₪{user_balances.get(member.id, 0):,}", inline=True); emb.add_field(name="כמות אזהרות:", value=f"{user_warns.get(member.id, 0)}/3", inline=True); emb.add_field(name="האם בכלא?", value="כן" if member.id in jail_list else "לא", inline=True); emb.add_field(name="הצטרף לשרת:", value=member.joined_at.strftime("%d/%m/%Y"), inline=False)
...
async def server_info(i):
    if not has_owner_or_admin_permission(i): [await send_unauthorized_alert(i, "server_info")]; return await i.response.send_message("❌ שגיאה: פקודה זו מוגדרת למנהלים ואונר בלבד!", ephemeral=True)
...
async def on_app_command_completion(i, cmd):
    log_ch = i.guild.get_channel(CHANNELS["OWNER_LOGS"])
    if log_ch:
        emb = discord.Embed(title="🛠️ לוג פקודות מערכת אוטומטי", color=0x5865f2, timestamp=datetime.now()); emb.add_field(name="מפעיל הפקודה:", value=i.user.mention, inline=True); emb.add_field(name="הפקודה שהורצה:", value=f"`/{cmd.name}`", inline=True); emb.add_field(name="הסבר פקודה:", value=cmd.description, inline=False)
        await log_ch.send(embed=emb)
